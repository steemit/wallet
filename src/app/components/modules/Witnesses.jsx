...
class Witnesses extends React.Component {
    ...
    componentDidMount() {
        if (this.props.walletSectionAccount && !this.props.current_proxy && !this.props.username) {
            const { fetchAccountWitnessVotes } = this.props;
            fetchAccountWitnessVotes(this.props.walletSectionAccount);
        }
    }

    componentDidUpdate(prevProps) {
        const { walletSectionAccount, fetchAccountWitnessVotes } = this.props;
        if (walletSectionAccount !== prevProps.walletSectionAccount) {
            if (walletSectionAccount && !this.props.current_proxy && !this.props.username) {
                fetchAccountWitnessVotes(walletSectionAccount);
            }
        }
    }
    ...
    render() {
        ...
        const witnesses = sorted_witnesses.map(item => {
            ...
            const isDisabled =
                signingKey == DISABLED_SIGNING_KEY;
                    ...
                    <td width="75">
                        ...
                        <span className={classUp}>
                            {votingActive ? (
                                ...
                                <a
                                href="#"
                                onClick={accountWitnessVote.bind(this, owner, !myVote)}
                                ...
                                <span
                                className="Voting__button Voting__button--disabled"
                                aria-disabled="true"
                                title={tt('loginform_jsx.login_to_vote')}
            ...
            <div className="Witnesses">
                <div className="row">
                    <div className="column">
                        ...
                        <span>
                            <span>
                                {tt('witnesses_jsx.witnesses_description')}
                                <FormattedHTMLMessage
                                    className="secondary inline-text"
                                    id="witnesses_jsx.witnesses_learn_more"
                                />
                            </span>
                        </span>
...
export default connect(
    (state, ownProps) => {
        ...
        const hasPropAccount = isNonEmptyString(ownProps.walletSectionAccount);
        const sourceAccountName = hasPropAccount
        ? ownProps.walletSectionAccount.trim()
        : username;

        const source_account = isNonEmptyString(sourceAccountName)
        ? state.global.getIn(['accounts', sourceAccountName])
        : null;

        const current_proxy = source_account ? source_account.get('proxy') : null;
        const witness_votes = source_account && source_account.get('witness_votes')
        ? source_account.get('witness_votes').toSet()
        : undefined;

        const witnessVotesInProgress = isNonEmptyString(sourceAccountName)
        ? state.global.get(
            `transaction_witness_vote_active_${sourceAccountName}`,
            Set()
            )
        : Set();
        ...
        const witnesses = state.global.get('witnesses', List());
        const canVote =
            !hasPropAccount || (isNonEmptyString(username) && norm(username) === norm(sourceAccountName));
    ...
    dispatch => {
        return {
            ...
            fetchAccountWitnessVotes: (accountName) => {
                return dispatch(transactionActions.fetchAccountWitnessVotes({accountName}))
            },
        };
    }
)(Witnesses)
